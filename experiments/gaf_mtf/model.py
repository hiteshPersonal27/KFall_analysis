"""
2D CNN for the GAF encoding experiment (see ../../docs/GAF_MTF_Implementation_Plan.md).
...
CNN+LSTM (unlike experiments/cwt_lstm/'s model) -- deliberate choice, not an
oversight: a GASF image has no time-frequency axis the way a scalogram does,
so there's no principled reason to treat its output as a sequence and feed
it into a recurrent/attention stage. This is a standard image-classification
structure, matching how 2D CNNs are conventionally used on this kind of
input (per the plan: "standard 2D CNN references, not KFall-specific papers,
are the right source for these choices").
...
small (50x50) image classification, not tuned specifically for this task.

Run standalone to print the shape progression and parameter count:
  python3 experiments/gaf_mtf/model.py
"""
