"""
4-panel version: ACC_M, Vertical Velocity, Tilt (pitch/roll), and Gyroscope magnitude.

Same phase-aligned event-locked averaging method as the 3-panel version
...
def compute_gyro_magnitude(df):
    gx = lowpass_filter(df["GyrX"].values)
    gy = lowpass_filter(df["GyrY"].values)
    gz = lowpass_filter(df["GyrZ"].values)
    ...
    for subject in subjects:
        ...
        for task in FALL_TASK_IDS:
            for trial in range(1, 6):
                ...
                gyro_m = compute_gyro_magnitude(df)
                ...
                gyro_p = resample_to_phase(frames, gyro_m, onset, impact)
                if acc_m_p is None:
                    continue

                records.append({
                    "subject": subject, "task": task, "trial": trial,
                    "acc_m": acc_m_p, "vv": vv_p, "tilt": tilt_p, "gyro_m": gyro_p,
                })
    return records
...
def plot_grand_average(records):
    ...
    gyro_m = np.array([r["gyro_m"] for r in records])
    ...
    specs = [
        ...
        (gyro_m, "Gyro Magnitude (deg/s)", "#9b59b6", None, None),
    ]
    ...
    axes[0].set_title(f"Grand-Average Fall Signature, Phase-Aligned (n={len(records)} trials)\n"
                       "0% = labeled onset, 100% = labeled impact  |  4-panel: ACC_M, VV, Tilt, Gyro")
...
def plot_intersubject_overlay(records):
    ...
    fig, axes = plt.subplots(4, 1, figsize=(11, 13), sharex=True)
    specs = [
        (acc_m, "ACC_M (g)"), (vv, "Vertical Velocity (m/s)"),
        (tilt, "Tilt (deg)"), (gyro_m, "Gyro Magnitude (deg/s)"),
    ]
    ...
    axes[0].set_title(f"Inter-Subject Overlay: each colored line = one subject's mean "
                       f"(n={len(subjects)} subjects)  |  4-panel: ACC_M, VV, Tilt, Gyro")
    axes[-1].set_xlabel("Phase (% of onset->impact duration)")
    plt.tight_layout()
...
def plot_intrasubject_small_multiples(records):
    # Kept ACC_M-focused, same as the 3-panel version: a 32-panel grid duplicated
    # per signal would clutter rather than clarify. See plan addendum for rationale.
    df_meta = pd.DataFrame([{"subject": r["subject"]} for r in records])
    acc_m = np.array([r["acc_m"] for r in records])
    subjects = sorted(df_meta["subject"].unique())
...
def plot_trial_heatmap(records):
    # Kept ACC_M-focused, same as the 3-panel version -- see rationale above.
...
if __name__ == "__main__":
    ...
    print("Building phase-aligned dataset from all fall trials (incl. gyro magnitude)...")
